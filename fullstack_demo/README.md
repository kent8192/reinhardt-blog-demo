...
```bash
# Create a new app
cargo run --bin fullstack_demo startapp myapp --with-pages

# Database migrations (when using database features)
cargo run --bin fullstack_demo makemigrations
cargo run --bin fullstack_demo migrate

# Check project for issues
cargo run --bin fullstack_demo check
```
