...
```bash
# Create a new app
cargo run --bin polls_project startapp myapp --with-pages

# Database migrations (when using database features)
cargo run --bin polls_project makemigrations
cargo run --bin polls_project migrate

# Check project for issues
cargo run --bin polls_project check
```
